        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setTitle(
                `Nenhuma música relacionada encontrada para \`${queue?.songs[0].name}\``
              ),
          ],
        })
        .then((msg) => {
      ...
      message.channel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`Gomen😕, não consigo procurar por: \`${quary}\``),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
        ...
        queue.textChannel
          .send({
            embeds: [
              new EmbedBuilder()
                .setColor(client.config.embed.color)
                .setDescription("Bom...já que não tem ninguem. vou sair aqui!"),
            ],
          })
          .then((msg) => {
            setTimeout(() => {
          ...
          case "autoplay":
            {
              if (!channel) {
                return send(
                  interaction,
                  `** ${client.config.emoji.ERROR} Você prescisa estar em um canal de voz**`
                );
              } else if (
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} Você precisa entrar no canal __My__ Voice `
                );
              } else if (!queue) {
                return send(
                  interaction,
                ...
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} Você não é o Deejay! e Você não deve soclicitar musicas no momento 🤨`
                );
          ...
          case "skip":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} No momento não estou tocando nada! `
                );
              } else if (checkDJ) {
                return send(
                  interaction,
          ...
          case "pauseresume":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} A faixa está pausada!! `
                );
              }
            }
            break;
          case "loop":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} modo de repetição desativado !!`
                );
              }
            }
            break;
      ...
      if (message.channelId === musicchannel.id) {
        ...
        if (message.author.bot) {
          ...
          if (
            ...
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} Eu não tenho permissão para entrar no canal de voz **`
            );
